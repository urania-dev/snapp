...
export const GET = async (event) => {
  ...
  return json({
    snapps,
    total: count,
    pagination: { limit, offset, query, orderBy, ascending },
  });
};
...
export const POST = async (event) => {
  ...
  if (userId && token.user.role === "user") {
    return error(403, {
      message: "You're not allowed to create snapps for someone else",
    });
  }

  const [snapp, err] = await database.snapps.create(
    {
      shortcode: slugify(shortcode),
      original_url,
      secret,
      max_usages,
      notes,
  ...
  if (err === MAX_SNAPPS_PER_USER) {
    message = "This account reached its limit number of snapps.";
  }
  if (err === SNAPP_ORIGIN_URL_REQUESTED) {
    message = "You must provide an Original URL";
  }
  if (err === SNAPP_ORIGIN_URL_BLACKLISTED) {
    message = "Original URL is blacklisted";
  }
  if (err === ALLOW_UNSECURE_HTTP) {
    message = "The original URL must be a secure https link";
  }
  ...
  const [snapp, err] = await database.snapps.edit(
    {
      ...
      expiration: expiration === null
        ? null
        : expiration || editable.expiration,
...
export const DELETE = async (event) => {
  ...
  const [count, err] = await database.snapps.delete(
    token.userId,
    ...ids.split(","),
  );
