<script lang="ts">
	...
	import { createTag, updateURL } from '$lib/remotes/urls.remote.js';
	...
	let hasLimits = $state(data.url?.limit !== null && data.url?.limit > -1);
	// svelte-ignore state_referenced_locally
	let hasPassword = $state(data.url.secret !== null);
	// svelte-ignore state_referenced_locally
	let hasExpiration = $state(data.url.expiresAt !== null);
	...
	// svelte-ignore state_referenced_locally
	let notes = $state<string | undefined>(data.url.notes || undefined);
	let triggerRefTags = $state<HTMLButtonElement>(null!);
	let triggerRefTeams = $state<HTMLButtonElement>(null!);

	// svelte-ignore state_referenced_locally
	let tags = $state<string[]>(data.url.tags.map((t) => t.id) || []);
	// svelte-ignore state_referenced_locally
	let teams = $state<string[]>(data.url.teams.map((t) => t.id) || []);
	...
	let utms = $state<Record<'key' | 'name' | 'value', string>[]>(
		// eslint-disable-next-line svelte/no-unused-svelte-ignore
		// svelte-ignore state_referenced_locally
		(!!data.url.utm && Object.values(data.url.utm)) || []
	);
	let expirationUnit = $state('minutes');
	let _exp = $state<null | number | undefined>(
		// eslint-disable-next-line svelte/no-unused-svelte-ignore
		// svelte-ignore state_referenced_locally
		(data.url.expiresAt &&
			Math.max(
				Math.floor((new Date().getTime() - data.url.expiresAt?.getTime()) / 1000 / 60),
				0
			)) ||
			undefined
	);

	let _secret = $state<null | string>();
	...
	let removeExpiration = $derived(_exp === null);
	let removeSecret = $derived(_secret === null);
</script>

<form
	class="@container flex w-full grow flex-col"
	{...updateURL.enhance(async ({ submit }) => {
		try {
			await submit();
		} catch (error) {
			console.error(error);
...
>
	<div class="flex sticky top-0 bg-background! z-20 h-14 w-full items-center gap-4 border-b p-4">
		...
		<Button type="submit" class="ms-auto"
			><SaveIcon /><span class="hidden md:block">{m.save_changes()}</span></Button
		>
	</div>
	<div class="grid w-full @3xl:grid-cols-2 @3xl:divide-y-0 divide-y @3xl:divide-x h-full">
		<Field.Set class="flex gap-0 flex-col grow divide-y">
			<div class="p-4 h-30">
				<Field.Field data-invalid={Array.isArray(updateURL.fields.originalUrl.issues())}>
					<Field.Label class="mb-1">{m.original_url()}</Field.Label>
					<Field.Content>
						<InputGroup.Root>
							<InputGroup.Addon><GlobeIcon /></InputGroup.Addon>
							<InputGroup.Input
								aria-invalid={updateURL.fields.originalUrl.as('url')['aria-invalid']}
								name={updateURL.fields.originalUrl.as('url')['name']}
								value={data.url.originalUrl}
								placeholder={m.original_url_placeholder()}
							/>
						</InputGroup.Root>
						{#each updateURL.fields.originalUrl.issues() as issue (issue)}
			...
			<div class="p-4 h-30">
				<Field.Field data-invalid={Array.isArray(updateURL.fields.shortcode.issues())}>
					...
					<InputGroup.Root>
						...
						<InputGroup.Input
							...
							aria-invalid={updateURL.fields.shortcode.as('url')['aria-invalid']}
							name={updateURL.fields.shortcode.as('url')['name']}
							value={data.url.shortcode}
							oninput={(e) => (e.currentTarget.value = slugify(e.currentTarget.value))}
						/>
					</InputGroup.Root>
					{#each updateURL.fields.shortcode.issues() as issue (issue)}
						<Field.Error>{issue.message}</Field.Error>
					{:else}
				...
				<div class="p-4 w-full">
					<Field.Field>
						...
						<Popover.Root bind:open={showTags}>
							<Popover.Trigger
								bind:ref={triggerRefTags}
								class={buttonVariants({ class: 'justify-between items-enter', variant: 'outline' })}
							>
				...
				<div class="p-4 w-full">
					<Field.Field>
						...
						<Popover.Root bind:open={showTeams}>
							<Popover.Trigger
								bind:ref={triggerRefTeams}
								class={buttonVariants({ class: 'justify-between items-enter', variant: 'outline' })}
							>
								{selectedTeams.join(', ') || m.select()}
							</Popover.Trigger>
		...
		<div class="flex flex-col w-full h-full divide-y">
			<div class="p-4 @lg:mt-auto min-h-30">
				<Field.Label for="hasExpiration" class="h-full">
			...
			{#if hasExpiration}
				<div class="p-4" in:fly={{ y: 12 }}>
					...
					<Field.Field class="flex flex-col">
						...
						{#each updateURL.fields.limit.issues() as issue (issue)}
							<Field.Error>{issue.message}</Field.Error>
						{:else}
							<Field.Description>{m.expiration_helper()}</Field.Description>
						{/each}
					</Field.Field>
				</div>
			{/if}
						...
						<Field.Content>
							<InputGroup.Root>
								...
								<InputGroup.Input
									...
									pattern="\d+"
									style="appearance: textfield"
									name={updateURL.fields.limit.as('text').name}
									aria-invalid={updateURL.fields.limit.as('text')['aria-invalid']}
									placeholder={m.limits_placeholder()}
									value={data.url.limit}
								/>
							</InputGroup.Root>
							{#each updateURL.fields.limit.issues() as issue (issue)}
								<Field.Error>{issue.message}</Field.Error>
							{:else}
								<Field.Description>{m.set_limits_helper()}</Field.Description>
							{/each}
						</Field.Content>
					</Field.Field>
				</div>
			{/if}
			<div class="p-4">
				<Field.Label for="hasSecret" class="text-balance min-h-20">
					<Field.Field orientation="horizontal">
						<Field.Content>
						...
						<Field.Content>
							<ButtonGroup class="w-full">
								<InputGroup.Root>
									...
									<InputGroup.Input
										name={updateURL.fields._secret.as('password').name}
										type={showPassword ? 'text' : 'password'}
										aria-invalid={updateURL.fields._secret.as('password')['aria-invalid']}
			...
			<div class="p-4 flex flex-col grow shrink-0">
	...
	<input hidden name="removeSecret" value={removeSecret} />
	<input hidden name="removeExpiration" value={removeExpiration} />
</form>
