...
<div class="page">
	<div class="flex">
		<div class="flex flex-col gap-4">
			<Breadcrumbs urls={[{ label: $t('global:pages:dashboard') }]} />
		...
		<a href="/dashboard/urls/import/" class="btn ms-auto self-center variant-glass flex">
			<ImportIcon class="w-4 h-4" />
			<Small class="font-semibold">
				{$t('snapps:import')}
			</Small>
		</a>
		<a href="/dashboard/shorten/" class="btn ms-4 self-center variant-filled-primary flex">
	...
	<div class="card w-full">
		...
		<div class="card-body px-4 flex items-center justify-between h-14">
								...
								{#if $active_columns.includes(column.id)}
									<th style:padding-inline=".5rem" class={column.classes} style:padding-block="0">
										{#if sortable_list.includes(column.id)}
											<button
												class="p-0 py-2 inline-flex items-center gap-1"
												data-column={column.id}
												on:click={sort_by}
											>
										...
										{:else}
											<button
												class="p-0 inline-flex justify-center items-center text-center cursor-default"
											>
												<Paragraph class="font-semibold leading-none table-header">
													{column.label}
												</Paragraph>
						...
						{#each data.urls as row, i}
							...
							>
								...
								{#if $active_columns.includes('shortcode')}<td>
										<a
											href="/dashboard/urls/{row.shortcode}"
								...
								{/if}
								{#if $active_columns.includes('ttl')}
									<td>
										{#if row.ttl === -1}
