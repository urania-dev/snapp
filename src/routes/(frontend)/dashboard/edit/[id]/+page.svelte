<script lang="ts">
	...
	let original_url: string = data.url?.original_url;
	let shortcode: string = data.url?.shortcode;
	let max_usages: number = data.url?.max_usages;
	...
	let enable_ttl = data.url?.ttl !== undefined;
	let enable_max_usages = data.url?.max_usages !== undefined;
	let enable_secret = data.url?.has_secret;
	...
	const saved_ttl = new Date(data.url.ttl);
	let date_picker_value: DateValue | undefined = new CalendarDate(
		saved_ttl.getFullYear(),
		saved_ttl.getMonth(),
		saved_ttl.getDate()
	);
	...
	let notes: string | undefined = data.url.notes;
	...
	const enhanceShorten: SubmitFunction = function ({ formData, cancel }) {
		...
		else formData.set('ttl', String(-1));
...
<svelte:head
	><title>{$t('global:appname')} | {$t('global:pages:edit', { id: data.url.id })}</title
	></svelte:head
>

<form id="edit" method="post" action="?/edit" use:enhance={enhanceShorten} />
<div class="page">
	<Breadcrumbs
		urls={[
			{ label: $t('global:pages:dashboard'), href: '/dashboard' },
			{ label: $t('global:pages:edit', { id: data.url.id }) }
		]}
	/>
	<div class="flex gap-2 items-center">
		<PlusIcon class="w-6 h-6" />
		<H3 class="mb-1 w-max">
			{$t('global:sections:edit')}
		</H3>
	</div>
		...
		<div class="flex flex-col gap-4">
			<div class="card w-full">
				<div class="card-header flex items-center justify-between h-14 py-0">
					<Lead>{$t('global:sections:edit:header')}</Lead>
				</div>
			...
			>
				...
				<div class="card-body full flex-grow hide-scrollbar flex gap-4 ps-[1px] pe-[1px] pb-[1px]">
					<textarea
						name="notes"
						class="textarea border-r-0 border-l-0 border-b-0 rounded-tl-none rounded-tr-none min-h-60 lg:min-h-[unset]"
						bind:value={notes}
					/>
				</div>
			</div>
			...
			<div class="card w-full">
				...
				<div class="card-body p-4 flex flex-col gap-4">
					<label for="ttl" class="flex flex-col gap-1 w-full">
						<Paragraph>{@html $t('snapps:sections:advanced:ttl:description')}</Paragraph>
						<div
						...
						>
							...
							{#if show_secret}
								<input
									...
									name="secret"
									placeholder={$t('snapps:sections:advanced:secret:edit:placeholder')}
									bind:value={secret}
								/>
							{:else}
								<input
