<script lang="ts">
	...
	import { H3, H4, Lead, Paragraph, Small } from '$lib/ui/typography';
	...
	let sortable_list = ['email', 'username'];
	...
	const columns: {
		...
		columnBuilder('username', 'w-auto'),
		columnBuilder('email', ' w-auto'),
		columnBuilder('roles', ' min-w-28'),
		columnBuilder('urls', ' m-w-10'),
		columnBuilder('created', ' min-w-28'),
		columnBuilder('updated', ' min-w-28')
	];
	...
	function columnBuilder(column: string, classes: string = 'text-center') {
		return {
			id: column,
			classes,
			label: $t(`users:columns:${column.replace('_', ':')}`),
	...
	function handle_active_id(this: HTMLInputElement) {
		const row = this.dataset.idx;
		active_id.set(data.users.find((u) => u.id === row));
	}

	function handle_select_id(this: HTMLInputElement) {
		const row = this.value;
		if (this.dataset.selectAll && $active_rows.length < data.users.length)
			return ($active_rows = data.users.map((u) => u.id));
		else if (this.dataset.selectAll && $active_rows.length === data.users.length)
			return ($active_rows = []);
	...
	$: modal = {
		type: 'confirm',
		// Data
		title: $t('users:selected:remove'),
		body: $t('users:selected:remove:helper', { ids_count: `${$active_rows.length}` }),
	...
	let active_id = writable<DBUser | undefined>();
...
<svelte:head><title>{$t('global:appname')} | {$t('global:pages:users')}</title></svelte:head>
...
<div data-popup="morePopup" class="z-20">
	<div class="card flex flex-col p-1">
		<a href="/dashboard/users/edit/{$active_id?.id}" class="btn variant-glass more-icon"
			>{$t('users:more:edit')}</a
		>
		<button class="btn variant-glass more-icon" on:click={handle_delete_single}
			>{$t('users:more:delete')}</button
		>
	</div>
</div>
	...
	<div class="flex justify-between">
		<div class="flex flex-col gap-4">
			...
			<div class="flex gap-2 items-center">
				...
				<H3 class="mb-1 w-max">
					{$t('global:pages:users')}
				</H3>
			</div>
		</div>
		<a href="/dashboard/users/create" class="btn self-center variant-filled-primary flex">
			...
			<div class="table-container rounded-none">
				...
				<table class="table table-hover variant-glass rounded-none">
					<thead class="align-middle max-h-12 overflow-hidden">
						<tr>
							<th style:padding-inline=".75rem" style:padding-block="0" class="w-10">
								<input
									type="checkbox"
									class="checkbox variant-outline-secondary active:variant-glass"
									on:change={handle_select_id}
									checked={data.users.length === $active_rows.length}
								...
								{#if $active_columns.includes(column.id)}
									<th style:padding-inline=".5rem" class={column.classes} style:padding-block="0">
										{#if sortable_list.includes(column.id)}
											<button
												class="p-2 px-0 inline-flex items-center gap-1"
												data-column={column.id}
												on:click={sort_by}
											>
					...
					<tbody>
						{#each data.users as row, i}
							...
							>
								...
								{#if $active_columns.includes('username')}
									<td class="cursor-default w-10" title={row.username}>
										<a href="/dashboard/users/{row.username}" class="link">
								...
								{/if}
								{#if $active_columns.includes('email')}
									<td class="cursor-default w-10" title={row.email}>
										<Small class="whitespace-nowrap">
											{row.email ?? ''}
										</Small>
									</td>
								{/if}
								{#if $active_columns.includes('roles')}
									<td class="cursor-default w-10">
										<div class="flex gap-2">
											{#if row.roles.includes('superadmin') || row.roles.includes('admin')}
												{#if row.roles.includes('superadmin')}
													<div class="badge badge-glass variant-glass-primary">
														{$t('users:role:superadmin')}
													</div>
												{/if}
												{#if row.roles.includes('admin')}
													<div class="badge badge-glass variant-glass-secondary">
														{$t('users:role:admin')}
													</div>
												{/if}
											{:else}
												<div class="badge badge-glass variant-glass">{$t('users:role:user')}</div>
											{/if}
										</div>
									</td>
								{/if}

								{#if $active_columns.includes('urls')}
								...
								{/if}
								{#if $active_columns.includes('created')}
									<td>
										<Small class="whitespace-nowrap">
								...
								{/if}
								{#if $active_columns.includes('updated')}
									<td>
										<Small class="whitespace-nowrap">
											{getRelativeDate(row.updated)}
										</Small>
									</td>
								{/if}
								...
								<td>
									<button
										class="btn ms-auto h-8 w-8 p-0 flex place-items-center variant-outline-primary"
						...
						{/each}
						{#each { length: data.limit - data.users.length } as _row, _index}
							<tr>
