...
export const load = async (
  { locals: { session, user }, params: { id }, url },
) => {
  ...
  const base = new Date();
  const start = startString ? new Date(startString) : new Date(base);
  const end = endString ? new Date(endString) : new Date(base);
  ...
  const limit = customLimit !== -1
    ? customLimit
    : parseInt(url.searchParams.get("limit")?.toString() || "4");
  const page = parseInt(url.searchParams.get("page")?.toString() || "1");
  const offset = (page - 1) * limit;

  const query = url.searchParams.get("query")?.toString();

  const [tags, countTag] = await database.tags.get(user.id, query, 5, offset, {
    name: "asc",
  });
  const tagsInSnapp = await prisma.tag.findMany({
    where: { snapps: { some: { id: snapp.id } } },
  });
  return {
    snapp,
    metrics,
...
export const actions = {
  "handle-tag": async (
    { locals: { session }, request, params: { id } },
  ) => {
