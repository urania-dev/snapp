<script lang="ts">
	...
	const loadQRCode = async () => {
		...
		const url = new SvelteURL(withPrefix);
...
<Separator />
<div
	class="grid-details grid h-full max-h-[calc(100%_-_80px)] w-full max-w-[calc(100%_-_1px)] grid-rows-[repeat(2,1fr)] content-start overflow-y-scroll xl:grid-cols-2"
>
	...
	>
		...
		<div class="grid h-full gap-2 lg:grid-cols-2">
			...
			>
				...
				<UTMParams showHelper={false} showFooter={false} params={utmParams} />
			</div>
		</div>
	</div>
</div>
