...
<div class={cn('flex w-full flex-col p-4 pb-8')}>
	<div class="mx-auto flex h-full w-full max-w-5xl flex-col gap-4">
		<a class="flex gap-2 font-semibold uppercase tracking-wider" href="/dashboard/{_snapp.id}"
		...
		<Card css={{ card: 'gap-4' }}>
			...
			<div class="mt-auto flex w-full gap-4">
				<a
					href="/dashboard/{_snapp.id}"
					class="flex h-10 w-full items-center gap-2 justify-center rounded border-none bg-slate-500/25 outline-none transition-all hover:bg-slate-500/50 focus:bg-slate-500/50"
				...
				<button
					...
					><Icon ph="floppy-disk" size={16} /><small class="font-semibold text-sm">
						{$_('globals.save')}
					</small>
				</button>
			</div>
