...
export const load = async ({
  ...
  url,
  request,
  params: { prefix, shortcode },
}) => {
  if (!shortcode) error(404, { message: "errors.snapps.not-found" });

  const [redirection,] = await database.snapps.one(shortcode);
  await prisma.snapp.findFirst({
    where: { shortcode, tags: { some: { slug: prefix } } },
  });
  if (!redirection) throw error(404, { message: "errors.snapps.not-found" });
  if (redirection.disabled === true) {
    throw error(404, { message: "errors.snapps.disabled" });
  }
  if (redirection !== null) {
    const is_allowed = await database.snapps.validate(
      redirection.original_url,
      fetch,
    );
...
export const actions = {
  async default(
    { locals: { user, }, request, params: { shortcode }, fetch, url },
  ) {
    const form = await request.formData();
    const secret = form.get("secret") as string | null;
    if (secret) {
      const [snapp, err] = await database.snapps.one(shortcode);
      if (err === SNAPP_DISABLED) {
        return fail(400, { message: "errors.snapps.disabled" });
      }
      ...
      if (validPassword) {
        if (!user || user.id !== snapp.userId) {
          await markUsage(snapp, request, url, fetch);
        }
