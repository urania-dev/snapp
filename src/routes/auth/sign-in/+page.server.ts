...
export const actions = {
  default: async ({ request, cookies }) => {
    ...
    if (!is_username_allowed) {
      return fail(400, { message: "errors.blacklisted.user" });
    }

    const [, err] = await database.users.authenticate(
      cookies,
      username,
      password,
    );
    ...
    let message = null;
    if (err && err === "PASSWORD_IS_INVALID") {
      message = "errors.auth.wrong-credentials";
    }
    if (err && err === "USER_DOES_NOT_EXISTS") {
      message = "errors.auth.user-not-found";
    }
