...
export const load = async (
  { locals: { session, user }, params: { slug }, url },
) => {
  ...
  const tag = await prisma.tag.findFirst({
    ...
    ...t,
    snapps: t ? t.snapps.map((s) => ({ ...s, secret: s.secret !== null })) : [],
  }));
  ...
  const [users] = is_admin
    ? await database.users.get(userQuery, 5, 0)
    : [[], 0];
...
export const actions = {
  "handle-user": async (
    { locals: { session }, request, params: { slug } },
  ) => {
  ...
  "save-cols": async ({ locals: { session, user }, request }) => {
    ...
    if (cols) {
      await database.settings.set("SNAPP_IN_TAGS_COLUMNS", cols, user.id);
    }
  },
