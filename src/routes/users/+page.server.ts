...
export async function load({ locals: { session, user }, url }) {
	...
	const enabled_limits = database.settings.parse(await database.settings.get(ENABLE_LIMITS), true);
	const defaultMaxSnapp = enabled_limits
		? await database.settings.get(MAX_SNAPPS_PER_USER).then((res) => res?.value || 0)
		: 0;
...
export const actions = {
	...
	edit: async ({ locals: { session }, request }) => {
		...
		if (_user.max)
			await database.settings.set(MAX_SNAPPS_PER_USER, _user.max.toString(), updated.id);
	...
	create: async ({ locals: { session }, request, url, fetch }) => {
		...
		const _user = JSON.parse(create_user) as User & { max: number | null | undefined };
		...
		try {
			...
			const html = String((await import('$lib/emails/invited.html?raw')).default);
