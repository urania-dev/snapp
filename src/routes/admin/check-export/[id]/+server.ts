...
export const GET = async ({ locals: { prisma, user }, params: { id } }) => {
	...
	if (!existsSync(csvPath)) {
		if (DEBUG) log.info("CSV does not exist. Creating a new one...");
	...
	try {
		...
		if (DEBUG) log.info("File modification time: " + new Date(fileStat.mtimeMs));
		if (DEBUG) log.info("Threshold time: " + new Date(tenMinutesAgo));

		if (fileStat.mtimeMs < tenMinutesAgo) {
			if (DEBUG) log.info("CSV is older than 10 minutes. Rebuilding...");
			await createCSV(user, id, prisma, csvPath);
			return json({ available: false });
		}

		if (DEBUG) log.info("CSV is fresh. No need to rebuild.");
		return json({ available: true });
	} catch (err) {
		if (DEBUG) log.info({ err, message: "Error checking file stats:" });
		return json({ available: false });
	}
};
