<script lang="ts">
	...
	import { Lead, Paragraph, Small } from '$lib/ui/typography';
	...
	function handle_submit(this: HTMLButtonElement) {
		document.forms.namedItem('add-domain')?.requestSubmit();
	}
</script>

<div class="card w-full container max-w-md">
	<div class="card-header flex items-center justify-between h-14 py-0 border-solid">
		<Lead>{$t('settings:app:blacklists:domain:add')}</Lead>
	</div>
	<form
		id="add-domain"
		method="post"
		action="/dashboard/settings/admin/blacklists?/banDomain"
		use:enhance={enhanceDomainUnBan}
	>
		<div class="card-body p-4 flex flex-col">
			<label for="domain" class="flex flex-col gap-1 mb-2">
				<div class="input-group variant-glass input-group-divider grid-cols-[auto_1fr_auto]">
					...
					<input
						...
						class:input-error={$page.form?.domain}
						id="domain"
						on:input={handle_enter_submit}
						type="text"
						name="domain"
						placeholder={$t('settings:app:blacklists:domain:placeholder')}
						bind:value={domain}
					/>
				</div>
			</label>
