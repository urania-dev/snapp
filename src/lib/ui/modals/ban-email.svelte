<script lang="ts">
	...
	const enhanceEmailUnBan: SubmitFunction = function ({ formData }) {
...
<div class="card w-full container max-w-md">
	<div class="card-header flex items-center justify-between h-14 py-0">
		<Lead>{$t('settings:app:blacklists:email:add')}</Lead>
	</div>
	<form
		id="add-email"
		method="post"
		action="/dashboard/settings/admin/blacklists?/banEmail"
		use:enhance={enhanceEmailUnBan}
	>
			...
			<label for="email" class="flex flex-col gap-1 mb-2">
				<div class="input-group variant-glass input-group-divider grid-cols-[auto_1fr_auto]">
					...
					<input
						...
						type="text"
						name="email"
						placeholder={$t('settings:app:blacklists:email:placeholder')}
						bind:value={email}
					/>
				</div>
			</label>
