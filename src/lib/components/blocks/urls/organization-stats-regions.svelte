<script lang="ts">
	import type { RegionVisit } from '$lib/server/metrics/helpers';
	...
	const { metrics = [] }: { metrics: RegionVisit[] } = $props();

	type Agg = { region: string; visitors: number };
	...
	const aggregatedRaw = $derived.by(() => {
		...
		for (const row of metrics) {
			const rawName = row.region?.trim()?.split('/')[0] || m.other();
		...
		return [...map.entries()].map(([region, visitors]) => ({
			region,
			visitors
		})) satisfies Agg[];
	});
	...
	const aggregated = $derived.by(() => {
		const sorted = aggregatedRaw.sort((a, b) => b.visitors - a.visitors);
	...
	type Stat = { color: string; region: string; visitors: number };
...
<Card.Root class="h-full w-full">
	<Card.Header>
		<Card.Title>{m.metrics_regions()}</Card.Title>
			...
			<Chart.Container config={chartConfig} class="h-62! aspect-auto">
				<BarChart
					...
					props={{
						...
						yAxis: {
							tickLabelProps: {
								class: 'stroke-none max-w-20 fill-primary',
								dx: -100,
								textAnchor: 'start'
							},
							tickLength: 0
