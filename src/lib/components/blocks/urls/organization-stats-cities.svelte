<script lang="ts">
	import type { CityVisit } from '$lib/server/metrics/helpers';
	...
	const { metrics = [] }: { metrics: CityVisit[] } = $props();

	type Agg = { city: string; visitors: number };
	...
	const aggregatedRaw = $derived.by(() => {
		...
		for (const row of metrics) {
			const rawName = row.city?.trim() || m.other();
		...
		return [...map.entries()].map(([city, visitors]) => ({
			city,
			visitors
		})) satisfies Agg[];
	});
	...
	const aggregated = $derived.by(() => {
		...
		const otherIndex = sorted.findIndex(
			(x) => x.city.toLowerCase() === 'other' || x.city === m.other()
		);
	...
	type Stat = { city: string; color: string; visitors: number };
...
<Card.Root class="h-full w-full">
	<Card.Header>
		<Card.Title>{m.metrics_cities()}</Card.Title>
