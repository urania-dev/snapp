<script lang="ts">
	import type { LanguageVisit } from '$lib/server/metrics/helpers';
	...
	const { metrics = [] }: { metrics: LanguageVisit[] } = $props();

	type Agg = { language: string; visitors: number };
	...
	const aggregatedRaw = $derived.by(() => {
		...
		for (const row of metrics) {
			const rawName = row.language?.trim() || m.other();

			let key = rawName.split('-')[0].trim().toLowerCase();
			if (key === 'other') key = m.other();
			const visits = (row.internal ?? 0) + (row.external ?? 0);

			map.set(key, (map.get(key) ?? 0) + visits);
		}

		return [...map.entries()].map(([language, visitors]) => ({
			language,
			visitors
		})) satisfies Agg[];
	});
	...
	const aggregated = $derived.by(() => {
		...
		const otherIndex = sorted.findIndex(
			(x) => x.language.toLowerCase() === 'other' || x.language === m.other()
		);

		if (otherIndex !== -1) sorted.splice(otherIndex, 1);

		if (sorted.length > 5) sorted.length = 5;

		return sorted.filter((s) => /^[A-Za-z]{2}$/.test(s.language));
	});
	...
	const chartConfig = $derived.by(() => {
		...
		aggregated.forEach((row, i) => {
			cfg[row.language] = {
				color: nordColor(i),
				label: translateLanguage(getLocale(), row.language)
			};
		});
	...
	type Stat = { color: string; language: string; visitors: number };

	const chartData = $derived.by(
		() =>
			aggregated.map((row, i) => ({
				color: nordColor(i),
				language: translateLanguage(getLocale(), row.language),
...
<Card.Root class="h-full w-full">
	<Card.Header>
		...
		<Card.Description>{m.metrics_languages_helper()}</Card.Description>
	</Card.Header>
