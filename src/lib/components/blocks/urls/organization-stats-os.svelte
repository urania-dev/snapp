<script lang="ts">
	import type { OsVisit } from '$lib/server/metrics/helpers';
	...
	const { metrics = [] }: { metrics: OsVisit[] } = $props();

	type Agg = { os: string; visitors: number };
	...
	const aggregatedRaw = $derived.by(() => {
		...
		for (const row of metrics) {
			const rawName = row.os?.trim() || m.other();
		...
		return [...map.entries()].map(([os, visitors]) => ({
			os,
			visitors
		})) satisfies Agg[];
	});
	...
	const aggregated = $derived.by(() => {
		...
		const otherIndex = sorted.findIndex(
			(x) => x.os.toLowerCase() === 'other' || x.os === m.other()
		);
	...
	type Stat = { color: string; os: string; visitors: number };
...
<Card.Root class="h-full w-full">
	<Card.Header>
		<Card.Title>{m.metrics_os()}</Card.Title>
		<Card.Description>{m.metrics_os_helper()}</Card.Description>
	</Card.Header>
