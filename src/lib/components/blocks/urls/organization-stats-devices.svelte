<script lang="ts">
	import type { DeviceVisit } from '$lib/server/metrics/helpers';
	...
	const { metrics = [] }: { metrics: DeviceVisit[] } = $props();

	type Agg = { device: string; visitors: number };
	...
	const aggregatedRaw = $derived.by(() => {
		...
		for (const row of metrics) {
			const rawName = row.device?.trim() || m.other();
		...
		return [...map.entries()].map(([device, visitors]) => ({
			device,
			visitors
		})) satisfies Agg[];
	});
	...
	const aggregated = $derived.by(() => {
		...
		const otherIndex = sorted.findIndex(
			(x) => x.device.toLowerCase() === 'other' || x.device === m.other()
		);
	...
	type Stat = { color: string; device: string; visitors: number };
...
<Card.Root class="h-full w-full">
	<Card.Header>
		<Card.Title>{m.metrics_devices()}</Card.Title>
		<Card.Description>{m.metrics_devices_helper()}</Card.Description>
	</Card.Header>
