...
import type { UserLogEntry } from '$lib/server/metrics/helpers';
	...
	},
	{
		...
		cell: ({ row }) => {
			const snippet = createRawSnippet<[{ date: Date }]>((get) => {
				...
				return { render: () => `<span class="pe-2">${formatted}</span>` };
			});
	...
	},
	{
		...
		cell: ({ row }) =>
			renderComponent(Code, {
				code: JSON.stringify(row.original.detail, null, 2)
			}),
		enableSorting: false,
		header: () => m.details()
	}
] satisfies ColumnDef<UserLogEntry>[];
