<script lang="ts" generics="TData, TValue">
	...
	const table = createSvelteTable({
		...
		getRowId(originalRow, index) {
			return (originalRow as { id: string }).id ?? index.toString();
		},
		...
		onPaginationChange: (u) => (paginationState = typeof u === 'function' ? u(paginationState) : u),
		onRowSelectionChange: (u) => {
			rowSelection = typeof u === 'function' ? u(rowSelection) : u;
			selected = Object.keys(rowSelection)[0] || null;
		},
		onSortingChange: (u) => (sortingState = typeof u === 'function' ? u(sortingState) : u),
		state: {
			get columnFilters() {
...
<div class="@container flex w-full grow flex-col divide-y">
	...
	<div class="flex h-full w-full flex-col p-4">
		<div class="h-full rounded-md border overflow-y-scroll overflow-clip max-h-80!">
