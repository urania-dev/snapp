<script lang="ts" generics="TData, TValue">
	...
	import TableDeleteUserModal from './table-delete-user-modal.svelte';
...
<div class="@container flex w-full grow flex-col divide-y">
	<div class="flex w-full flex-col items-center justify-between @lg:flex-row">
		<div class="flex w-full flex-col items-center px-4 pt-4 @lg:py-4">
			<Field.Field orientation="responsive" class="w-full max-w-max me-auto">
				<Field.Label class="sr-only" for="search">{m.users_search_placeholder()}</Field.Label>
				...
				{#each table.getAllColumns().filter((col) => col.getCanHide()) as column (column.id)}
					<DropdownMenu.CheckboxItem
						...
						onCheckedChange={async () => {
							await updateColumns({ columns: columnVisibility, cookie: 'user-columns' });
							await invalidateAll();
						}}
						...
						<Table.Row>
							{#each headerGroup.headers as header (header.id)}
								<Table.Head
									...
									class={[
										['members', 'users'].includes(header.id) && 'w-20!',
										'id' === header.id && 'w-8',
					...
					{#each table.getRowModel().rows as row (row.id)}
						...
						>
							{#each row.getVisibleCells() as cell (cell.id)}
								<Table.Cell class="h-10! py-1 pe-1 {cell.column.id === 'actions' && 'w-10 p-0!'}">
							...
							<Table.Cell colspan={columns.length} class="content-center text-center ">
								<Empty.Root>
									<Empty.Header>
										...
										<Empty.Title>{m.users()}</Empty.Title>
										<Empty.Description>
											{m.no_results()}
										</Empty.Description>
									</Empty.Header>
				...
				<InputGroup.Root class="h-8! w-full!">
					...
					<InputGroup.Input
						...
						oninput={() => {
							if (!limit) return;
							d.debounce(async () => {
								await updateRows({ cookie: 'user-rows', rows: limit.toString() });
							}, 500)();
						}}
						bind:value={limit}
					/>
