<script lang="ts" generics="TData, TValue">
	...
	import TableDeleteTeamModal from './table-delete-team-modal.svelte';
	...
	<div class="flex w-full flex-col items-center justify-between @lg:flex-row">
		<div class="flex w-full flex-col items-center px-4 pt-4 @lg:py-4">
			<Field.Field orientation="responsive" class="w-full max-w-max me-auto">
				<Field.Label class="sr-only" for="search">{m.teams_search_placeholder()}</Field.Label>
				...
				{#each table.getAllColumns().filter((col) => col.getCanHide()) as column (column.id)}
					<DropdownMenu.CheckboxItem
						...
						onCheckedChange={async () => {
							await updateColumns({ columns: columnVisibility, cookie: 'team-columns' });
							await invalidateAll();
						}}
				...
				<InputGroup.Root class="h-8! w-full!">
					...
					<InputGroup.Input
						...
						oninput={() => {
							...
							d.debounce(async () => {
								await updateRows({ cookie: 'team-rows', rows: limit.toString() });
							}, 500)();
						}}
						bind:value={limit}
					/>
