<script lang="ts">
	...
	let whiteListQueryEmails = $derived(
		JSON.stringify({
			skip,
			take,
			where:
				query.trim() !== ''
					...
					: { allowed: true, domain: { not: null }, username: { not: null } }
		})
	);
	let whiteListQueryDomains = $derived(
		JSON.stringify({
			skip,
			take,
			where:
...
>
	<Tabs.Content value="whiteList" class="w-full">
		<Form bind:lastUpdate {whiteListForm}></Form>
						...
						<Table.Root class="mb-0">
							...
							<Table.Body>
								{#key lastUpdate}
									...
									{:then whitelistedItems}
										{#each whitelistedItems as item (item.id)}
												...
												<Table.Cell class="p-2 text-right">
													<Button
														...
														onclick={async () => {
															...
															const idx = whitelistedItems.findIndex((i) => i.id === item.id);
															if (idx > -1) whitelistedItems.splice(idx, 1);
														}}
													>
														<i class="ph ph-trash text-[20px]"></i>
														<span class="sr-only">Un-whitelist this user</span>
													</Button>
												</Table.Cell>
											</Table.Row>
										{:else}
