				...
				const getContent = createRawSnippet<[{ id: string; shortcode: string | undefined }]>(
					(getProps) => {
						...
						return {
							render: () =>
								isPrivateView
									? `<a href="/dashboard/${id}" class="text-xs flex justify-center font-medium hover:text-foreground w-full text-center whitespace-nowrap text-nowrap text-ellipsis">${shortcode}</a>`
									: `<a href="/${shortcode}" data-sveltekit-preload-data="off" class="text-xs flex shrink-0 justify-center font-medium hover:text-foreground w-full whitespace-nowrap text-nowrap text-ellipsis text-center">${shortcode}</a>`
						};
					}
				);
