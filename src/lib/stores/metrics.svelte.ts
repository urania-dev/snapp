...
class MetricsQueryStore {
	...
	weekAgo = new SvelteDate(Date.now() - A_WEEK);
	...
	url = $derived(
		`/api/usage/groupBy?q=${JSON.stringify({
			...
			where: {
				timestamp: {
					gte: new SvelteDate(this.start.toDate(getLocalTimeZone())),
					lte: new SvelteDate(this.end.toDate(getLocalTimeZone()))
				}
			}
		})}`
	);
	...
	loadFromDb = async () => {
		...
		const end = new SvelteDate(this.end.toDate(getLocalTimeZone()));
		...
		try {
			...
			res.data?.forEach((u) => {
				const date = new SvelteDate(u.timestamp).toLocaleDateString(page.data.locale, {
					day: "numeric",
					month: "numeric",
			...
			for (
				let d = new SvelteDate(this.start.toDate(getLocalTimeZone()));
				d <= new SvelteDate(this.end.toDate(getLocalTimeZone()));
	...
	setEnd = (date: SvelteDate = new SvelteDate()) =>
		(this.end = new CalendarDate(date.getFullYear(), date.getMonth() + 1, date.getDate() + 1));
	setStart = (date: SvelteDate = new SvelteDate()) =>
