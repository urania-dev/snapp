class SlidingWindowCounter {
  ...
  private getCurrentBucket = (apiKey: string): number => {
    ...
    const currentBucket = Math.floor(
      timeSinceLastReset / (this.windowSize / this.bucketCount),
    );
...
function getRateLimiterInstance(
  ...
  if (!globalThis[instanceKey]) {
    globalThis[instanceKey] = new SlidingWindowCounter();
  }
...
const rateLimiterCheck = async (token: string) => {
  ...
  if (rpdBlocked || rpmBlocked) {
    ...
    const remainingTime = Math.floor(
      Math.max(
        rpdBlocked ? rpdRemainingTime : 0,
        rpmBlocked ? rpmRemainingTime : 0,
      ) / 1000,
    );
...
export { rateLimiterCheck, RPD, RPM };
